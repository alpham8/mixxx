...
void WSpinny::draw() {
    ...
    // Must come after cue glow and reset clip path
    {
        ...
        if (bpm > 0.0 && width() >= 80) {
            ...
            // Draw the centred labels with a soft dark shadow so they stay
            // readable on top of the (white) needle and the cover art.
            ...
            // Pitch % (left) and tempo range (right), just below the BPM
            ...
            // Elapsed and remaining time, below the centre hub
            if (trackSamples > 0.0 && sampleRate > 0.0) {
                ...
                // Remaining time below elapsed time (Serato-style countdown)
